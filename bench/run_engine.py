...
#     """
#     Normalize arbitrary values into a safe text string.

#     - None -> ""
#     - str -> unchanged
#     - bytes -> decoded utf-8
#     - anything else -> str(value), with final fallback to ""
#     """
#     if value is None:
#         return ""
...
#     """
#     Normalize token counts or similar numeric fields.
#     """
#     try:
#         if value is None:
#             return default
...
#     fn.write_text(_coerce_text(html), encoding="utf-8")
#     return fn
...
#        variant: template: "Insert a {component} for “{label}”{attribute}"
#        test: {"component":"text field","label":"...","attribute":" with hint ..."}
#     """
...
#     attribute = str(test.get("attribute") or "").strip()
...
#     fields = {"component": component, "label": label, "attribute": attribute}
...
#                             test_row = tests[i]
#                             comp_title = str(test_row.get("title", f"Component {i+1}") or f"Component {i+1}")

#                             user_prompt = _prompt_from_test_variant(test_row, vid, v_row).strip()
...
#                                         gen_error = _coerce_text(getattr(gen, "error", ""))
#                                         gen_content = _coerce_text(getattr(gen, "content", ""))
#                                         prompt_tokens = _coerce_int(getattr(gen, "prompt_tokens", 0), default=0)
#                                         completion_tokens = _coerce_int(getattr(gen, "completion_tokens", 0), default=0)
...
#                                 if gen_ok:
#                                     safe_gen_content = _coerce_text(gen_content)
...
#                                         if should_cancel():
#                                             raise RunCancelled()
#                                         raw, mx, per = score_checks(safe_gen_content, checks, reg.score)
#                                         raw_score = float(raw)
#                                         max_score = float(mx)
...
# azure 
from bench.azure import AzureFoundryClient as OpenRouterClient, calculate_cost
